#!/bin/bash
#SBATCH --job-name=qutrit_rb_terminal_check
#SBATCH --output=qutrit_rb_terminal_check_%A_%a.out
#SBATCH --error=qutrit_rb_terminal_check_%A_%a.err
#SBATCH --array=0-19
#SBATCH -C cpu
#SBATCH -q regular
#SBATCH -t 47:30:00
...
if [ "${SLURM_ARRAY_TASK_ID}" -lt "${#error_rates[@]}" ]; then
    ...
    echo "No configured error rate for task index ${SLURM_ARRAY_TASK_ID}."
fi
